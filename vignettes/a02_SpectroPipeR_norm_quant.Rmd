...
<u>**data normalization:**</u> - normalize the ion data using the median-median lagorithm or aout-detect if the data was already normalized in Spectronaut using e.g. local cross-run normalization
...
16. generate sample to condition file including measurement order
17. generate plots & table outputs
...
ComBat allows users to adjust for batch effects in datasets where the batch covariate is known, using methodology described in Johnson *et al.* 2007. It uses either parametric (used by SpectroPipeR) or non-parametric empirical Bayes frameworks for adjusting data for batch effects.

“The ComBat batch adjustment approach assumes that batch effects represent non-biological but systematic shifts in the mean or variability of proteomic features for all samples within a processing batch. … This procedure pools information across genes in each batch to shrink the batch effect parameter estimates toward the overall mean of the batch effect empirical estimates. These are used to adjust the data for batch effects. This approach provides a robust and often more accurate adjustment for the batch effect on each protein.” - modified from Zhang et al., 2018
...
![](figures/folder_structure_normalize.png){width="30%"}

![](figures/folder_structure_processed.png){width="50%"}
...
![](figures/02__normalization_factor_plot.png){width="50%"}
...
![](figures/02__normalization_factor_BOXplot.png){width="50%"}
...
![](figures/02__normalization_boxplot.png){width="50%"}
...
![](figures/02__ion_CV_plot.png){width="50%"}
...
![](figures/02__ion_intenisty_vs_CV_hexbin_plot.png){width="80%"}
...
![](figures/02a__MaxLFQ_protein_intensity_boxplot.png){width="50%"}
...
![](figures/02a__CV_vs_intensity_plot.png){width="80%"}
...
![](figures/02a__protein_intensities_vs_iBAQ_intensities.png){width="70%"}
...
![](figures/02a__CV_cumulative_frequency_plot.png){width="70%"}
...
The **sample_to_condition_file.csv** table contains the information about the run file name, condtion, replicate, run date and resulting measurement order.
...
Since the a protein group can contain more than one protein the number of theoretical peptides may differ. Therefore Spectronaut separates with a ";" e.g. "6179.5;6376.5". 
SpectroPipeR uses the mean of the iBAQ values to give an iBAQ estimate for the protein group as well.
...
-   <u>R.Condtion	</u> conditions as provided by Spectronaut report
