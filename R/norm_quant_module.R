...
norm_quant_module <- function(SpectroPipeR_data = NULL,
  ...
  #MS_level check
  if(MS_level%in%c("MS1","MS2")){
  }else{
    message_function(text = "no valid MS level is used. use MS2 or MS2 only.",color = "red",log_file_name = log_file_name)
    stop()
  }
  ...
  # add MS level to paramters
  parameter_input$MS_level <- MS_level
...
# select MS level for quantification --------------------------------------
  if(MS_level=="MS1"){
    data$`EG.TotalQuantity (Settings)`  <- data$FG.MS1Quantity
  }
  if(MS_level=="MS2"){
    data$`EG.TotalQuantity (Settings)`  <- data$FG.MS2Quantity
  }
  ...
  if(sum(data$EG.NormalizationFactor,na.rm = T)!=0){
    ...
    if(parameter$normalization_method=="median"){
      ...
      if(MS_level=="MS1"){
        med <- median(data$FG.MS1Quantity,
                      na.rm = T)
      }
      if(MS_level=="MS2"){
        med <- median(data$FG.MS2Quantity,
                      na.rm = T)
      }
