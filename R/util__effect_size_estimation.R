    ...
    for(k in 1:length(unique_PG)){
      ...
      tmp_single_out_bind <- bind_rows(tmp_single_out_bind,
                                       tibble(PG.ProteinGroups = unique_PG[k],
                                              ...
                                              effect_size_method = ifelse(test = is.na(tmp_cohend$estimate),
                                                                          yes = "none - equal variance",
                                                                          no = tmp_cohend$method),
                                              d = ifelse(test = is.na(tmp_cohend$estimate),
                                                         yes = 0,
                                                         no = tmp_cohend$estimate),
                                              d_pooled_SD = ifelse(test = is.na(tmp_cohend$sd),
                                                                   yes = 0,
                                                                   no = tmp_cohend$sd),
                                              d_95CI_lower = ifelse(test = is.na(tmp_cohend$conf.int[1]),
                                                                    yes = 0,
                                                                    no = tmp_cohend$conf.int[1]),
                                              d_95CI_upper = ifelse(test = is.na(tmp_cohend$conf.int[2]),
                                                                    yes = 0,
